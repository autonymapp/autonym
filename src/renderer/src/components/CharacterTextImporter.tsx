...
export default function CharacterTextImporter({
  onApply
}: {
  ...
  async function structure(): Promise<void> {
    ...
    setError(null)
    try {
      const fields = await window.api.characters.structureText(rawText)
      onApply(fields)
      setOpen(false)
    } catch (err: any) {
      setError(friendlyError(err))
  ...
  return (
    <div className="panel" style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <p className="hint" style={{ margin: 0 }}>
        Paste reference text (from a wiki, your own notes, anything) — the AI organizes it into
        this character's sheet for you to review. Works for an existing canon character or an OC.
      </p>
